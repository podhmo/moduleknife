...
class timeHandler:
    ...
    def emit(self, m, graph, src, dst):
        m.stmt('{} -> {} [label="{}"]', graph[src], graph[dst], round(graph.metadata[dst], 5))
...
def main():
    ...
    parser.add_argument("--metadata", choices=["time"], default="time")
    parser.add_argument(
        "--driver",
    ...
    metadata_handler = getattr(globals(), args.metadata + "Handler", timeHandler)()
